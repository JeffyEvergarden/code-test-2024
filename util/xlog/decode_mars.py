...
def AppendText(_outbuffer, text):
    _outbuffer.extend(text.encode("utf-8", errors="replace"))
...
def DecodeBuffer(_buffer, _offset, _outbuffer):
    ...
    if not ret[0]:
        ...
        if -1==fixpos: 
            ...
            AppendText(_outbuffer, "[F]decode_log_file.py decode error len=%d, result:%s \n"%(fixpos, ret[1]))
            _offset += fixpos 
    ...
    if MAGIC_NO_COMPRESS_START==magic_start or MAGIC_COMPRESS_START==magic_start or MAGIC_COMPRESS_START1==magic_start:
        ...
        AppendText(_outbuffer, 'in DecodeBuffer _buffer[%d]:%d != MAGIC_NUM_START'%(_offset, magic_start))
        return -1
    ...
    begin_hour = struct.unpack_from("B", _buffer, _offset + headerLen - 4 - crypt_key_len - 1 - 1)[0]
    end_hour = struct.unpack_from("B", _buffer, _offset + headerLen - 4 - crypt_key_len - 1)[0]

    global lastseq
    if seq != 0 and seq != 1 and lastseq != 0 and seq != (lastseq+1):
        AppendText(_outbuffer, "[F]decode_log_file.py log seq:%d-%d is missing\n" %(lastseq+1, seq-1))
    ...
    try:
        ...
        if MAGIC_NO_COMPRESS_START1==_buffer[_offset] or MAGIC_COMPRESS_START2==_buffer[_offset] or MAGIC_SYNC_ZSTD_START==_buffer[_offset] or MAGIC_ASYNC_ZSTD_START==_buffer[_offset]:
            ...
            if zstd is None:
                raise RuntimeError("zstandard is required for zstd-compressed xlog files")
        ...
        traceback.print_exc()  
        AppendText(_outbuffer, "[F]decode_log_file.py decompress err, " + str(e) + "\n")
